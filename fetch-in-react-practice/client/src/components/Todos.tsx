      ...
      try {
        ...
        if (!response.ok) {
          setError({
            ...
            message: 'Request Failed at fetchAllTodos',
          });
        }
        ...
        setError({ statusCode: 500, message: 'Network error' });
      }
    }
    fetchAllTodos();
  }, []);
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      ...
      if (!response.ok) {
        setError({
          statusCode: response.status,
          message: 'Request Failed at addTodo',
        });
      }
  ...
  async function toggleCompleted(todo: Todo) {
    try {
      ...
      if (!response.ok) {
        setError({
          statusCode: response.status,
          message: 'Request Failed at toggleCompleted',
        });
      }

      const updatedTodo = await response.json();
      const todoList = todos.map((oldTodo) =>
        oldTodo.todoId === updatedTodo.todoId ? updatedTodo : oldTodo
      );
